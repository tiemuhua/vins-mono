...
namespace vins {
    ...
    public:
         ...
         * 将待优化变量在时间上分为：
         * 1.原始量x_original:
         *   上一轮优化过程结束后，边缘化过程中，即addMetaFactor()和marginalize()时对应的数据.
         *   保存在MarginalMetaFactor.parameter_blocks_中，指向slide_window_estimator.cpp中c_pos、c_vel等数组
         * 2.更新量x_updated:
         ...
         * 3.冻结量x_frozen:
         *   由于c_pos、c_vel中的数据在后续优化中被不断更新，
         *   marginalize()在reserve_block_frozen_中储存了一份保留原始量的快照，称之为冻结量。
         *   冻结量在下一轮优化中保持不变。
         *   @param reserve_block_origin 持有保留原始量，且顺序上和reserve_block_frozen_一一对应，其内容同样是c_pos、c_vel等数组。
         *
         * 优化过程：
         *   marginalize()计算残差R和保留原始量的雅可比J，保存在reserve_param_residuals_和reserve_param_jacobians_中
